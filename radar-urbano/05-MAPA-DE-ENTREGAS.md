...
| 05 | Mapa de Domínio | Vendas & Performance | Dia 1 (reduzido no Dia 0 e dia 7) | Imagem no WhatsApp; interativo no dashboard | Cartógrafo |
...
| 08 | Extrato de Demanda | Controladoria & Eficiência | Dia 1 | PDF no WhatsApp; série no dashboard | Tesoureiro |
| 09 | Horas Devolvidas | Controladoria & Eficiência | Dia 1 | Card no WhatsApp | Tesoureiro |
| 10 | Voz do Cliente | Controladoria & Eficiência | Dia 1; alerta quando um tema dispara | Card no WhatsApp | Anfitrião + Redator-chefe |
| 11 | Fechamento Executivo | Controladoria & Eficiência | Dia 1; trimestral | PDF no WhatsApp; arquivo no dashboard | Redator-chefe |

Envios: Boletim segunda 7h; dia 1 e dia 7 até as 8h; linhas de uma frase e perguntas Sim/Não das 8h às 19h. Reenvio se o WhatsApp não confirmar em 2 h, sem duplicar.

## 3. As entregas

### 01 Diagnóstico de Posição

Assim que o dono adiciona o Radar Urbano como Gerente, a máquina mede 2–3 termos-chave em nove pontos, audita a completude do perfil e manda o card na mesma conversa. Referência de todo Antes e Depois.
...
Perfil do Dia 0 e do dia 7 lado a lado, campo a campo (diff de `profile_audits`: campo, antes, depois, origem), com a segunda medição reduzida nos mesmos termos e pontos e as duas datas.
...
Painel aberto pelo link fixo (com token; vira ícone no celular). Sem menu, filtro ou configuração. Contém os quatro contatos e o total com seta semanal, impressões e palavras-chave, posição nos termos-chave, reputação, Diário com prints e custo por contato.
...
- Senha opcional criada pelo dono; link mágico entra sem senha; ninguém do Radar Urbano vê a senha. Números só de SQL.

### 04 Boletim de Segunda

Áudio de até 60 s com a voz da casa e card de uma tela, toda segunda às 7h, inclusive feriado. Cobre a semana fechada pelo Google (até a data de referência, tipicamente quinta ou sexta anterior) e diz isso em uma frase: contatos, avaliações novas e respondidas, posição nos termos-chave (sábado 3h), ações do Diário, o que vem.

- Segunda 4h post; 6h Redator-chefe confere cada número contra o banco e escreve no tom do Dia 0; TTS (Chirp 3 HD pt-BR, `voice_id` fixo, OGG/Opus); 7h envio.
- Janela de 24 h fechada: modelo `boletim_segunda` (card + voz em MP4); aberta: PNG e OGG livres.
...
- Places API dentro dos termos: nada guardado além de 30 dias exceto `place_id`; a medição própria fica na série. Sem raspagem.
- Cadência: Dia 0 e dia 7 reduzido (9 pontos); sábado 3h reduzido (alimenta Boletim e dashboard); dia 1 2h completo. Vermelhos viram `editor.tasks`.
...
- Feriados: checagem diária 5h; tabela própria de feriados nacionais e municipais por cidade; 72 h antes, horário especial no perfil e post de aviso, pela regra do dono; na dúvida, pergunta Sim/Não também 72 h antes.
- Categorias sempre com dois olhos; principal só com Sim do dono. Nunca termos de busca no nome.
- Linha no WhatsApp só para ação relevante (feriado, descrição reescrita, capa trocada), em horário comercial.

### 07 Motor de Reputação

Polling a cada 30 min. Resposta humanizada e específica a toda avaliação em até 2 h (mediana < 2 h, 100% em 24 h); perguntas públicas em menos de 4 h. Boletim de Reputação no dia 1 (nota, novas, respondidas e em quanto tempo, temas elogiados e de atenção).

- Balcão: link curto oficial "Receber mais avaliações" e QR em PDF, gerados na segunda semana. Só avaliações reais: sem incentivo, sem seleção, sem convite ativo por mensagem.
- Notas 1 e 2 passam pelo Guardião em qualquer patamar, ainda dentro das 2 h.
- Sem dado do avaliador; texto não armazenado (hash, temas, sentimento). Política de conteúdo do Google verificada antes de publicar.
- Avaliação suspeita (perfil recém-criado, texto repetido, série de notas baixas no dia) vira sinal para o Guardião denunciar pelo canal oficial; a máquina não discute em público.

### 08 Extrato de Demanda

Uma página por mês só com o que se conta: quatro linhas e a soma, custo por contato (R$ 499 ÷ contatos), contexto fora da soma (impressões, dez palavras-chave), série de seis meses. O dono coloca o valor dele por cima.
...
Trabalho executado, tirado linha a linha do Diário (nenhuma hora sem print) e convertido em tempo humano pela tabela abaixo, publicada no rodapé do card e fixa no mês. Tempo do dono = minutos das respostas Sim/Não registradas.
...
Avaliações e perguntas públicas do mês classificadas por tema e sentimento: três elogios e três atritos mais recorrentes, com frequência e uma frase real sem nome. A máquina não lê conversa de ninguém.
...
- Armazenado: tema, sentimento e hash; nunca texto nem nome. Lote noturno com Gemini Flash.

### 11 Fechamento Executivo

Uma página em PDF, último envio da manhã do dia 1: contatos contra o mês anterior, impressões, custo por contato, nota e avaliações, Mapa de Domínio em miniatura, horas devolvidas, o que a máquina fez e vai fazer, data de referência no rodapé.

- Trimestral, junto com o mensal: três meses lado a lado e a curva desde o Dia 0 (contatos, custo por contato, pontos verdes, nota).
...
|---|---|
| Dia 0 | Antes, fora dos 15 min: `zone.check`, contrato por link, cartão. Min 0–5: cinco perguntas por áudio. Min 5–8: Gerente no perfil. Min 8–10: link do dashboard, senha opcional, Diagnóstico. Opt-in registrado com hora no Sim final. |
| Semana 1 | Perfil auditado e corrigido; feriados do ano na agenda. Dashboard no ar; Google entra na madrugada seguinte. Pendências de reputação respondidas. Sábado 3h: medição reduzida. Dia 7: Antes e Depois. |
| Semana 2 | Primeiro Boletim com semana fechada. Post semanal em ritmo. QR em PDF. Segunda medição de sábado. |
| Semana 3 | Série de posição com 4–5 pontos; vermelhos viram tarefas do Editor. Primeiro feriado 72 h antes, se houver. Lote noturno classificando temas. |
| Semana 4 | Dia 1, 2h–8h: medição completa, auditoria, extratos, cards, PDFs, envios. Chegam as seis entregas mensais. |
| Mês 2 | Termos novos testados; esforço movido para o vermelho. Segunda auditoria. Voz do Cliente orienta correções. |
...
|---|---|---|
| Cartógrafo | Mede posição por termo e ponto (Places API), desenha o Mapa de Domínio, faz o Diagnóstico, diz ao Editor o que otimizar | Dia 0, dia 7, sábado 3h; dia 1 2h |
...
| Anfitrião | Responde avaliações e perguntas, classifica tema e sentimento, sinaliza avaliação suspeita, gera QR, escreve o Boletim de Reputação | 30 min, lote noturno, dia 1 |
...
| Redator-chefe | Dado em 60 s de áudio e uma página: Diagnóstico, Boletim, Voz do Cliente, Fechamento; confere números antes; lembra pendências com pergunta | Segunda 6h, dia 1 6h, trimestral |
